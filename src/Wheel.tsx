...
export const Wheel: React.FC<WheelProps> = ({
  ...
  const spinWheel = () => {
    ...
    const offsetAngle = rawIndex * segmentSize;
    const normalizedAngle = (offsetAngle - Math.PI / 2 + 2 * Math.PI) % (2 * Math.PI);
    const newAngle = normalizedAngle + Math.random() * segmentSize;; // or use it in animation logic
